  ...
  class Datatables
  {
    ...
    */
    private function get_paging()
    {
      ...
      $iStart = $this->ci->input->$request('start');
      $iLength = $this->ci->input->$request('length');
    ...
    */
    private function get_ordering()
    {
      ...
      $Data = $this->ci->input->$request('columns');


      if ($this->ci->input->$request('order'))
        foreach ($this->ci->input->$request('order') as $key)
          if($this->check_cType())
    ...
    */
    private function get_filtering()
    {
      ...
      $mColArray = $this->ci->input->$request('columns');
    
      $sWhere = '';
      $search = $this->ci->input->$request('search');
      //tambahan untuk tidak error di php 7.4
      if(!isset($search['value'])){
        $search['value'] = '';
      }
      //end tambahan untuk tidak error di php 7.4
    ...
    */
    private function produce_output($output, $charset)
    {
      ...
      if($output == 'json')
      {
        $sOutput = array
        (
          'draw'                => intval($this->ci->input->$request('draw')),
          'recordsTotal'        => $iTotal,
    ...
    */
    private function check_cType()
    {
      ...
      $column = $this->ci->input->$request('columns');
